...
class FollowPath(object):
    ahead_time = 1 #in seconds
    radius =0.01
    ahead_dist = 0.5
    path_arr = np.array([[1.0,-1.0],[1.0,1.0], [2.0,3.0], [5.0,3.0]])
    node_cnt=0
    first_time = True
    node_radius = 0.1
    ...
    def look_ahead(self):
        ...
        self.new_position.x += self.twist.linear.x*(np.sin(self.new_yaw)-np.sin(self.yaw))/self.twist.angular.z
        self.new_position.y += self.twist.linear.x*(-np.cos(self.new_yaw)+np.cos(self.yaw))/self.twist.angular.z
    ...
    def line_segment(self, pt):
        ...
        self.cnt = min_i
        seek = Seek(self.pose.position, self.yaw, self.projected_pt)
    ...
    def continue_path(self, pt):
        ...
        if short_d> self.radius:
            seek = Seek(self.pose.position, self.yaw, self.projected_pt)
        else:
            seek = Seek(self.pose.position, self.yaw, self.projected_pt, True)
...
class Seek(object):
    ...
    def reach(self):
        ...
        self.cmd_v.linear.x = 0.20
        print("Vel", self.cmd_v.linear.x)
    ...
    def steer(self): 
        ...
        if self.no_steer:
            return 0.0
        return np.clip(req_steer, -1.57, 1.57)


    def __init__(self, curr_position, yaw, target, no_steer=False):
        # print("Seeking")
        self.target = target
